...
def concatenate_input_noise_map(input, noise_sigma):
    ...
    # Fill the downsampled image with zeros (device-agnostic: works on cpu, cuda, mps)
...
class UpSampleFeaturesFunction(Function):
    ...
    def forward(ctx, input):
        ...
        result = torch.zeros(N, Cout, Hout, Wout, dtype=torch.float32, device=device)
        for idx in range(sca2):
    ...
    def backward(ctx, grad_output):
        ...
        # Build output
        grad_input = torch.zeros(N, Cg_in, Hg_in, Wg_in, dtype=torch.float32, device=device)
        # Populate output
        for idx in range(sca2):
