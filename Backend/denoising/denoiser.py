...
class FFDNetDenoiser:
    ...
    def get_denoised_image(self, imorig, sigma = None):
        ...
        if (max(imorig.shape[0], imorig.shape[1]) > 900):
            ratio = max(imorig.shape[0], imorig.shape[1]) / 900
        ...
        imorig = imorig.to(self.device)
        imnoisy = imorig.clone()

        with torch.no_grad():
            nsigma = torch.FloatTensor([cur_sigma]).to(self.device)
